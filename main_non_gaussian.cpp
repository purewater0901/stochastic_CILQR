...
struct LandMark {
  LandMark(const double _x, const double _y, const double _std_x,
           const double _std_y)
...
int main() {
  for (int i = 1; i <= 2; i++) {
    const int robot_num = i;
    for (int j = 1; j <= 5; j++) {
      const int data_num = j;
      // Creating Map
      std::map<int, int> barcode_map;
      std::ifstream barcode_file("../data/MRCLAM_Dataset" +
                                 std::to_string(data_num) + "/Barcodes.dat");
      ...
      std::ifstream landmark_file("../data/MRCLAM_Dataset" +
                                  std::to_string(data_num) +
      ...
      // Reading files
      const std::string odometry_filename =
          "../data/MRCLAM_Dataset" + std::to_string(data_num) + "/Robot" +
      ...
      nkf_state_info.mean = {ground_truth_x.front(), ground_truth_y.front(),
                             ground_truth_yaw.front()};
      ...
      for (size_t odo_id = 0; odo_id < 20000; ++odo_id) {
        ...
        if (measurement_id < measurement_time.size() &&
            next_time - measurement_time.at(measurement_id) > 0.0) {
          ...
          while (measurement_id < measurement_time.size()) {
            ...
            const double dt =
                measurement_time.at(measurement_id) - current_time;

            // predict
            if (dt > 1e-5) {
              const Eigen::Vector2d inputs = {odometry_v.at(odo_id) * dt,
                                              odometry_w.at(odo_id) * dt};
              const std::map<int, std::shared_ptr<BaseDistribution>>
                  system_noise_map = {{SYSTEM_NOISE::IDX::WV,
                                       std::make_shared<NormalDistribution>(
                                           mean_wv * dt, cov_wv * dt * dt)},
                                      {SYSTEM_NOISE::IDX::WU,
                                       std::make_shared<NormalDistribution>(
                                           mean_wu * dt, cov_wu * dt * dt)}};
              ekf_state_info =
                  ekf.predict(ekf_state_info, inputs, dt, system_noise_map);
              ukf_state_info =
                  ukf.predict(ukf_state_info, inputs, system_noise_map,
                              measurement_noise_map);
              nkf_state_info =
                  nkf.predict(nkf_state_info, inputs, system_noise_map);
            }

            // update
            const Eigen::Vector2d meas = {
                measurement_range[measurement_id],
                measurement_bearing[measurement_id]};  // measurement value
            const Eigen::Vector2d meas_noise = {wr_dist(generator),
                                                wa_dist(generator)};
            const Eigen::Vector2d y = {
                meas(0) * std::cos(meas(1) + meas_noise(1)) * meas_noise(0),
                meas(0) * std::sin(meas(1) + meas_noise(1)) *
                    meas_noise(0)};  // transform
            const auto landmark =
                landmark_map.at(measurement_subject.at(measurement_id));
            const double updated_dt = std::max(1e-5, dt);
            const std::map<int, std::shared_ptr<BaseDistribution>>
                system_noise_map = {{SYSTEM_NOISE::IDX::WV,
                                     std::make_shared<NormalDistribution>(
                                         mean_wv * updated_dt,
                                         cov_wv * updated_dt * updated_dt)},
                                    {SYSTEM_NOISE::IDX::WU,
                                     std::make_shared<NormalDistribution>(
                                         mean_wu * updated_dt,
                                         cov_wu * updated_dt * updated_dt)}};
            ekf_state_info =
                ekf.update(ekf_state_info, y, {landmark.x, landmark.y},
                           measurement_noise_map);
            ukf_state_info =
                ukf.update(ukf_state_info, y, {landmark.x, landmark.y},
                           system_noise_map, measurement_noise_map);
            nkf_state_info =
                nkf.update(nkf_state_info, y, {landmark.x, landmark.y},
                           measurement_noise_map);
        ...
        while (ground_truth_time.at(ground_truth_id) < current_time &&
               ground_truth_time.at(ground_truth_id) < next_time) {
          ++ground_truth_id;
        }

        if (current_time < ground_truth_time.at(ground_truth_id) &&
            ground_truth_time.at(ground_truth_id) < next_time) {
          while (true) {
            ...
            const double dt =
                ground_truth_time.at(ground_truth_id) - current_time;
            if (dt > 1e-5) {
              const Eigen::Vector2d inputs = {odometry_v.at(odo_id) * dt,
                                              odometry_w.at(odo_id) * dt};
              const std::map<int, std::shared_ptr<BaseDistribution>>
            ...
            // EKF
            {
              ...
              const double dyaw =
                  normalizeRadian(true_yaw - ekf_state_info.mean(2));
            ...
            // UKF
            {
              ...
              const double xy_error = std::hypot(dx, dy);
              const double dyaw =
                  normalizeRadian(true_yaw - ukf_state_info.mean(2));
            ...
            // NKF
            {
              ...
              const double xy_error = std::hypot(dx, dy);
              const double dyaw =
                  normalizeRadian(true_yaw - nkf_state_info.mean(2));
        ...
        if (dt > 1e-5) {
          ...
          const std::map<int, std::shared_ptr<BaseDistribution>>
              system_noise_map = {
                  {SYSTEM_NOISE::IDX::WV, std::make_shared<NormalDistribution>(
                                              mean_wv * dt, cov_wv * dt * dt)},
                  {SYSTEM_NOISE::IDX::WU, std::make_shared<NormalDistribution>(
                                              mean_wu * dt, cov_wu * dt * dt)}};
          ekf_state_info =
              ekf.predict(ekf_state_info, inputs, dt, system_noise_map);
          ukf_state_info = ukf.predict(ukf_state_info, inputs, system_noise_map,
                                       measurement_noise_map);
          nkf_state_info =
              nkf.predict(nkf_state_info, inputs, system_noise_map);
        }
      ...
      // Output data to file
      {
        ...
        parent_dir += "/dataset" + std::to_string(data_num) + "robot" +
                      std::to_string(robot_num);
        std::filesystem::create_directories(parent_dir);
        const std::string filename = parent_dir + scenario.filename_;
        outputResultToFile(
            filename, times, x_true_vec, y_true_vec, yaw_true_vec,
            nkf_x_estimate, nkf_y_estimate, nkf_yaw_estimate, ekf_x_estimate,
            ekf_y_estimate, ekf_yaw_estimate, ukf_x_estimate, ukf_y_estimate,
            ukf_yaw_estimate, nkf_xy_errors, nkf_yaw_errors, ekf_xy_errors,
            ekf_yaw_errors, ukf_xy_errors, ukf_yaw_errors);
      }

      std::cout << "ekf_xy_error mean: "
                << ekf_xy_error_sum / ekf_xy_errors.size() << std::endl;
      std::cout << "ukf_xy_error mean: "
                << ukf_xy_error_sum / ukf_xy_errors.size() << std::endl;
      std::cout << "nkf_xy_error mean: "
                << nkf_xy_error_sum / nkf_xy_errors.size() << std::endl;
      std::cout << "ekf_yaw_error mean: "
                << ekf_yaw_error_sum / ekf_yaw_errors.size() << std::endl;
      std::cout << "ukf_yaw_error mean: "
                << ukf_yaw_error_sum / ukf_yaw_errors.size() << std::endl;
      std::cout << "nkf_yaw_error mean: "
                << nkf_yaw_error_sum / nkf_yaw_errors.size() << std::endl;
      ...
      ekf_key1.insert(
          std::pair<std::string, std::string>("label", "ekf_error"));
      // nkf_key1.insert(std::pair<std::string, std::string>("label",
      // "nkf_error")); ukf_key1.insert(std::pair<std::string,
      // std::string>("label", "ukf_error"));
      ...
      matplotlibcpp::save("../result/dataset" + std::to_string(data_num) +
                          "robot" + std::to_string(robot_num) +
                          "_error_ng.png");
      ...
      ekf_key2.insert(
          std::pair<std::string, std::string>("label", "ekf_estimation"));
      // nkf_key2.insert(std::pair<std::string, std::string>("label",
      // "nkf_estimation")); ukf_key2.insert(std::pair<std::string,
      // std::string>("label", "ukf_estimation"));
      ...
      matplotlibcpp::save("../result/dataset" + std::to_string(data_num) +
                          "robot" + std::to_string(robot_num) + "_traj_ng.png");
    }
  }
  return 0;
}
